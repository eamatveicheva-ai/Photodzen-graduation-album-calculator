...
const T = {
  mgr_q:
    "Клиентский менеджер — это сотрудник, который консультирует потенциальных заказчиков по тарифам и условиям заказа, оформляет договоры, осуществляет коммуникацию с заказчиком и исполнителями на всех этапах выполнения заказа, согласовывает подготовленные дизайн-макеты с заказчиком, передаёт исполнителям необходимую информацию для внесения корректировок в дизайн-макеты, оформляет заказ в типографии, контролирует соблюдение сроков и качество результата, решает все спорные ситуации. Если этот функционал выполняешь ты сам, то отключи тумблер и укажи, во сколько ты оцениваешь свои трудозатраты на это.",
  mgr_rate:
            ...
            <Card style={{marginBottom:14}}>
              ...
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  <thead>
                    <tr>
                      <th style={{textAlign:"left",padding:"5px 8px",color:C.muted,fontWeight:400,borderBottom:`1px solid ${C.border}`}}>Параметр</th>
                      {tariffs.map((t,i)=>(
              ...
              <Card style={{marginBottom:14}}>
                ...
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                    <thead>
                      <tr>
                        <th style={{textAlign:"left",padding:"6px 10px",color:C.muted,fontWeight:400,borderBottom:`1px solid ${C.border}`}}>Показатель</th>
                        {tariffs.map((t,i)=>(
                    ...
                    <tbody>
                      {[
                        ...
                        <tr key={ri} style={{background:row.accent?`${C.c1}08`:""}}>
                          <td style={{padding:"6px 10px",color:row.accent?C.c1:row.bold?C.text:C.muted,fontWeight:row.bold||row.accent?600:400,borderBottom:`1px solid ${C.border}22`}} className="pm"><TT text={row.tip}>{row.label}</TT></td>
                          {row.vals.map((v,i)=>(
